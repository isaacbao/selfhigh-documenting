...
let getDocument = async(function (request, response) {
    ...
    for (let i = 0; i < documentJson.changeLogs.length; i++) {
        documentJson.changeLogs[i].date = new Date(documentJson.changeLogs[i].date);
    }
    documentJson.changeLogs.sort((log1, log2)=>{
        return log2.date.getTime() - log1.date.getTime();
    });
    for (let i = 0; i < documentJson.changeLogs.length; i++) {
        documentJson.changeLogs[i].date = documentJson.changeLogs[i].date.toString().slice(0, 10)
    }
